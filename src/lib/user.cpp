...
void User::setHostName(QString hostname) { hostName = hostname; }

void User::setUserName(QString username) { userName = username; }

void User::setUserPassword(QString password) { userPassword = password; }

void User::setUserPasswordConfirm(QString passwordConfirm) {
  userPasswordConfirm = passwordConfirm;
}

void User::setRootPassword(QString password) { rootPassword = password; }
...
QString User::getHostName() { return hostName; }

QString User::getUserName() { return userName; }

QString User::getUserPassword() { return userPassword; }

QString User::getUserPasswordConfirm() { return userPasswordConfirm; }

QString User::getRootPassword() { return rootPassword; }

QString User::getRootPasswordConfirm() { return rootPasswordConfirm; }

QString User::getError() { return error; }

bool User::getIsValid() { return isValid; }

bool isEmpty(QString text) { return text.trimmed().size() == 0; }

void User::validatePasswords(QString errorMessage, QString password,
...
void User::validateFields() {
  ...
  validatePasswords(tr("User passwords don't match"), userPassword,
                    userPasswordConfirm);
  validatePasswords(tr("Root passwords don't match"), rootPassword,
                    rootPasswordConfirm);
