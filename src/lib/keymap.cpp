...
QDir Keymap::keymapDir =
    QDir(QStringLiteral("/usr/share/kbd/keymaps/i386/qwerty"));

Keymap::Keymap(QObject *parent) : QObject(parent) {}

QString Keymap::getSelectedKeyboard() { return selectedKeyboard; }
...
QStringList Keymap::allKeyboards() {
  if (keyboardsCache.size() < 1) {
    keyboardsCache = keymapDir.entryList(QStringList() << "*.map.gz")
